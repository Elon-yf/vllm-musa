...
class qint8_t {
 ...
 public:
  __device__ int8_t as_int8() const { return _; }

  __host__ __device__ qint8_t(int8_t val) : _(val) {}
  __host__ __device__ qint8_t() : _(0) {}
  __host__ __device__ qint8_t(const qint8_t& b) { _ = b._; }
  __host__ __device__ qint8_t(const int& b) { _ = b; }
  __host__ __device__ qint8_t(const char& b) { _ = b; }
  __host__ __device__ operator char() const { return _; }
  __host__ __device__ qint8_t operator+=(const qint8_t& b) { return _ += b._; }
  __host__ __device__ qint8_t operator-=(const qint8_t& b) { return _ -= b._; }
  __host__ __device__ qint8_t operator*=(const qint8_t& b) { return _ *= b._; }

  bool operator<(const qint8_t& b) const { return _ < b._; }
  bool operator>(const qint8_t& b) const { return _ > b._; }
  bool operator==(const qint8_t& b) const { return _ == b._; }
  bool operator!=(const qint8_t& b) const { return _ != b._; }
};
