...
def test_musa_graph_flags_are_registered_for_compile_cache(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import vllm.envs as vllm_envs

    from vllm_musa import _register_musa_environment_variables
    ...
    missing = object()
    previous = {
        name: vllm_envs.environment_variables.pop(name, missing)
        for name in names
    }
    try:
        _register_musa_environment_variables()
        fallback_name = "VLLM_MUSA_SPEC_DECODE_RANDOM_FALLBACK"
        monkeypatch.setenv(fallback_name, "0")
        assert vllm_envs.environment_variables[fallback_name]() is False

        name = "VLLM_MUSA_GEMMA_FUSED_ADD_RMSNORM"
        getter = vllm_envs.environment_variables[name]
        ...
        for name, old_getter in previous.items():
            if old_getter is missing:
                vllm_envs.environment_variables.pop(name, None)
            else:
                vllm_envs.environment_variables[name] = old_getter
