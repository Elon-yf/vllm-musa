...
def test_setup_py_reads_pins_not_literal():
    ...
    assert "_read_pins(" in src and '_PINS["VLLM_TAG"]' in src, "setup.py must read VLLM_TAG from PINS"
...
def test_makefile_sync_reads_pins_not_literal():
    ...
    assert "FETCH_HEAD" not in mk, "Makefile.sync still references the old FETCH_HEAD pin"
